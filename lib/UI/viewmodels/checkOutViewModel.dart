...
class CheckOutViewModel extends BaseModel {
  ...
  void proceed() async {
    if (_orderService.cartData.orderList.length == 0 || checkingOut) return;
    ...
    if (sid == null) {
      _dialogService.showDialog(
          title: "Error", description: "Credit card information error");
      return;
    }
    ...
    for (int i = 0; i < serialised["orderList"].length; i++) {
      serialised["orderList"][i]["product"] =
          serialised["orderList"][i]["product"]["id"];
      serialised["orderList"][i]["uom"] =
          serialised["orderList"][i]["uom"]["id"];
      serialised["orderList"][i]["color"] =
          serialised["orderList"][i]["color"]["id"];
      serialised["orderList"][i]["size"] =
          serialised["orderList"][i]["size"]["id"];
    }

    try {
      String orderNumber =
          await _apiService.postOrder(jsonEncode(serialised), sid);
      _dialogService.showDialog(
          title: "success",
          description: "Your order #ORD$orderNumber was finished successfully");
      ...
      _dialogService.showDialog(
          title: "Error",
          description:
              "Your order coudn't be proceeded\n error : ${e.toString()}");
    }
