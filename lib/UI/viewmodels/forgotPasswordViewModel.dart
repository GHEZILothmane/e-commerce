...
class ForgotPasswordViewModel extends BaseModel {
  ...
  void handleEmailForm() async {
    ...
    if (!emailFormState.currentState.validate()) return;

    setState(ViewState.Busy);

    try {
      if (await _authentificationService
          .passwordReset({"email": email}, "generateResetToken")) {
        await _dialogService.showDialog(
            title: "Success",
            description:
                "An email has been sent to you, containing the code to reset your password");
  ...
  void handleTokenForm() async {
    ...
    if (!tokenFormState.currentState.validate()) return;

    setState(ViewState.Busy);

    try {
      if (await _authentificationService
          .passwordReset({"email": email, "token": token}, "checkResetToken")) {
        await _dialogService.showDialog(
            title: "Success",
            description: "Your password reset token is valid");
        pageController.nextPage(
            duration: Duration(milliseconds: 300), curve: Curves.linear);
      }
    } catch (e) {
  ...
  void handlePasswordForm() async {
    ...
    if (!passwordFormState.currentState.validate()) return;

    setState(ViewState.Busy);

    try {
      if (await _authentificationService.passwordReset(
