...
import 'package:ecommerce/core/models/userData.dart';
...
class CartViewModel extends BaseModel {
  ...
  init() async {
    ...
    _orderService
        .changeCart((order) => order.sameDayDelivery = showSameDayDelivery);
    notifyListeners();
  }
  ...
  Future<bool> checkDistance() async {
    ...
    if (currentAdressType == BillingAdressUsage.sameAdress) {
      ...
      adress =
          "${user.deliveryAdress}, ${user.deliveryCity}, ${user.deliveryState} ${user.deliveryZipCode}, ${user.deliveryCountry}";
    } else {
      adress =
          "${deliveryAdressC.text}, ${deliveryCityC.text}, ${deliveryStateC.text} ${deliveryZipCodeC.text}, ${deliveryCountryC.text}";
    }

    return await _apiService
        .getSetting("home_position", body: {"adress": adress});
  }
  ...
  onProductQuantityChanged(int index, int value) {
    ...
    orderData.orderList[index].price = (orderData.orderList[index].unitPrice *
  ...
  void calculateSummary() async {
    ...
    try {
      Map response =
          await _apiService.calculateOrderTotal(_orderService.cartData);
  ...
  void onPromoCodeClicked() async {
    ...
    if (promo != null) {
      ...
      _dialogService.showDialog(
          title: "promo code",
          description: "Couldn't retrive promo code or promo code inexisting");
    }
  ...
  void checkOut() async {
    if (!_authentificationService.isConnected) {
      await _dialogService.showDialog(
          title: "Error",
          description: "Please register before confirming the order");
      return;
    } else if (_orderService.cartData.orderList.length == 0) {
      await _dialogService.showDialog(
          title: "Error", description: "Please add products to the cart");
      return;
    }
    ...
    if (currentAdressType == BillingAdressUsage.newAdress) {
      if (!formState.currentState.validate()) {
        await _dialogService.showDialog(
            title: "Missing infos", description: "Please fill the adresses");
        return;
      }
      ...
      orderData.deliveryMobileNumber =
          _authentificationService.user.deliveryMobileNumber;
      orderData.billingMobileNumber =
          _authentificationService.user.billingMobileNumber;
