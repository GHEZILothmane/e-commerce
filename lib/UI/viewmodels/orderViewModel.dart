...
class OrderViewModel extends BaseModel {
  ...
  void init(OrderState state) =>
      orders = _authentificationService.user.orders.where((element) {
  ...
  void refund(int id) async {
    if (refunding &&
        !(await _dialogService.showConfirmationDialog(
                title: "Refunding",
                description: "Do you really want to refund this order"))
            .confirmed) return;
    ...
    try {
      ...
      _dialogService.showDialog(
          title: "Success", description: "Order refunded successfully");
  ...
  void onDetailShow(OrderData order) async {
    _navigationService.navigateTo(RoutesManager.orderViewDetail,
        arguments: order);
  }
}
