...
class ProfileViewModel extends BaseModel {
  ...
  void getUser() async {
    ...
    await Task(() => _authentificationService.getUser())
        .attempt()
        .mapLeftToFailure()
        .run()
  ...
  void logOut() async {
    if ((await _dialogService.showConfirmationDialog(
        ...
        .confirmed) _authentificationService.logOut();
    notifyListeners();
  }
      ...
      try {
        if (await _authentificationService.updateUser({}, image)) {
          await _dialogService.showDialog(
              title: "Success", description: "Profile image updated");
          getUser();
        }
      } catch (e) {
        await _dialogService.showDialog(
            title: "Error",
            description: "Error updating adresses : ${e.toString()}");
      }
    }
  }
  ...
  updateBillingAddress() async {
    ...
    try {
      ...
      if (await _authentificationService.updateUser(body, null)) {
        await _dialogService.showDialog(
            title: "Success", description: "Adresses updated");
        getUser();
      }
    } catch (e) {
      await _dialogService.showDialog(
  ...
  void updatePassword() async {
    DialogResponsePassword response =
        await _dialogService.showPasswordDialog(title: "Reset password");
    if (!response.confirmed) return;

    try {
      if (await _authentificationService.userPasswordReset(
          response.password, response.newPassword)) {
        await _dialogService.showDialog(
            title: "Success", description: "Password updated");
        getUser();
      }
    } catch (e) {
      await _dialogService.showDialog(
