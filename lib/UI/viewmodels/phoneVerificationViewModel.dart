...
class PhoneVerificationViewModel extends BaseModel {
  PhoneVerficationService _phoneVerficationService =
      locator<PhoneVerficationService>();
    ...
    try {
      if (await _phoneVerficationService.verifyPhone(code, token)) {
        _dialogService.showDialog(
            title: "Success", description: "Acoount phone confirmed");
  ...
  void resendMessage() async {
    ...
    try {
      if (await _phoneVerficationService.resendMessage(token)) {
        _dialogService.showDialog(
            title: "Success", description: "Message resent");
      }
    } catch (e) {
      await _dialogService.showDialog(
          title: "error", description: e.toString());
