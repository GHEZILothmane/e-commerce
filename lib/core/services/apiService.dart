...
class ApiService {
  ...
  Future<List<T>> getDatas<T>(
      {@required String urlPath,
      @required T Function(dynamic) perItemCreation}) async {
    try {
      List<T> li = List<T>();
      ...
      for (var item in json.decode(body)) li.add(perItemCreation(item));
  ...
  Future<T> getData<T>(
      {@required String urlPath,
      @required T Function(dynamic) itemCreation}) async {
    try {
      http.Response result = await client.get(
        "$endpoint/$urlPath",
      );
  ...
  Future<List<T>> getDatasWithAuth<T>(
      {@required String urlPath,
      @required T Function(dynamic) perItemCreation}) async {
    try {
      List<T> li = List<T>();
      http.Response result = await client.get("$endpoint/$urlPath", headers: {
  ...
  Future<double> getPromoCode(String code) async {
    try {
      http.Response result = await client
          .post("$endpoint/backend/checkPromoCode", body: {"code": code});
  ...
  Future<String> postOrder(dynamic order, String paymentId) async {
    try {
      http.Response result = await client.post("$endpoint/backend/postOrders",
          headers: {
            "Authorization": "TOKEN ${_authentificationService.token}",
  ...
  Future<Map> calculateOrderTotal(OrderData order) async {
    try {
      ...
      http.Response result =
          await client.post("$endpoint/backend/orderCalculateTotal",
              headers: {
                'Content-type': 'application/json',
              },
              body: jsonEncode({
  ...
  Future<dynamic> getSetting(String settingName, {Map body = const {}}) async {
    try {
      http.Response result = await client
          .post("$endpoint/settings/getSettings/$settingName", body: body);
