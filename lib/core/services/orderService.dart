...
class OrderService {
  ...
  bool checkIfRedeemExceed(ProductOrderData product) {
    ...
    for (ProductOrderData orderData in _cartData.orderList) {
      if (orderData.isRedeemed)
        points += orderData.product.redeemPoints *
            orderData.quantity *
            orderData.uom.value;
    }
  ...
  void loadData() {
    ...
    if (_wishList == null) _wishList = List<ProductData>();
  }
  ...
  void addItemToOrder(ProductOrderData product) {
    ...
    if (product.isRedeemed && checkIfRedeemExceed(product)) {
      _dialogService.showDialog(
          title: "Cart",
          description: "Not enough points to redeem this product");
      return;
    }
    ...
    if (product.product.quantity < product.quantity) {
      _dialogService.showDialog(
          title: "Cart", description: "Product is not available");
      return;
    }
    ...
    if (_cartData.orderList.indexWhere((element) {
          return element.product.id == product.product.id &&
              element.uom.id == product.uom.id;
        }) ==
        -1) {
      _cartData.orderList.add(product.copyWith());
      _localDatabaseService.setCart(_cartData);
      _dialogService.showDialog(
          title: "Cart", description: "Item added to cart");
    } else
      _dialogService.showDialog(
          title: "Cart", description: "Item already in cart");
  }
  ...
  void addItemToWishList(ProductData product) {
    if (!_wishList.contains(product)) {
      ...
      _localDatabaseService.setWishList(_wishList);
      _dialogService.showDialog(
          title: "Wish list", description: "Wish list item added");
    }
  }
