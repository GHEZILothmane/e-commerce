...
import 'package:ecommerce/core/models/userState.dart';
...
class AuthentificationService {
  ...
  Future<UserData> getUser() async {
    try {
      http.Response result = await ApiService.client
          .get("${ApiService.endpoint}/settings/getUser", headers: {
  ...
  Future<bool> connect(String auth, String password) async {
    try {
      http.Response result = await ApiService.client
          .post("${ApiService.endpoint}/settings/auth", body: {
        "auth": auth,
        "password": password,
      });
      ...
      if (result.statusCode != HttpStatus.ok) {
        ...
        switch (userState) {
          case UserState.needActivation:
            _navigationService.navigateTo(RoutesManager.phoneVerification,
  ...
  Future<String> postUser(Map<String, dynamic> user, File image) async {
    try {
      var request = http.MultipartRequest(
          'POST', Uri.parse("${ApiService.endpoint}/settings/registerUser"));
      ...
      if (result.statusCode != HttpStatus.created)
        throw Failure(
            "Coudn't register the account, email or username already exists");
      else
        return body["token"];
    } on Failure {
      rethrow;
  ...
  Future<bool> passwordReset(Map<String, String> body, String url) async {
    try {
      http.Response result = await ApiService.client
          .post("${ApiService.endpoint}/settings/$url", body: body);
  ...
  Future<bool> userPasswordReset(String password, String newPassword) async {
    try {
      http.Response result = await ApiService.client
          .post("${ApiService.endpoint}/settings/updateUserPassword", body: {
        "password": password,
  ...
  Future<bool> updateUser(Map<String, String> user, File image) async {
    try {
      var request = http.MultipartRequest(
          'PATCH', Uri.parse("${ApiService.endpoint}/settings/updateUser"));
